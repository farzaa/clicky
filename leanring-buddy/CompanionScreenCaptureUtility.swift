    ...
    static func captureAllScreensAsJPEG() async throws -> [CompanionScreenCapture] {
        do {
            ...
            if Self.isPermissionDeniedError(error) {
                ClickyDebugLogger.log("screen.capture", "ScreenCaptureKit denied by TCC", metadata: [
                    "error": error.localizedDescription
                ])
                throw CompanionScreenCaptureError.permissionDenied(underlying: error)
            }
            ClickyDebugLogger.log("screen.capture", "ScreenCaptureKit capture failed", metadata: [
                "error": error.localizedDescription
            ])
            throw error
        }
