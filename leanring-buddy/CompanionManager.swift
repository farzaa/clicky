...
final class CompanionManager: ObservableObject {
    ...
    /// buddy should fly to and point at. Parsed from the response;
    ...
    private lazy var aiChatAPI: AIChatAPI = {
        return AIChatAPI(proxyURL: "\(Self.workerBaseURL)/chat", model: selectedModel)
    }()
    ...
    /// Conversation history so the selected model remembers prior exchanges
    /// within a session.
    ...
    @Published var selectedModel: String = UserDefaults.standard.string(forKey: "selectedAIModel")
    ...
    func start() {
        ...
        // Eagerly touch the AI chat so its TLS warmup handshake completes
    ...
    private func sendTranscriptToSelectedModelWithScreenshot(transcript: String) {
        ...
        currentResponseTask = Task {
            ...
            do {
                ...
                let (fullResponseText, _) = try await aiChatAPI.analyzeImageStreaming(
    ...
    /// Result of parsing a [POINT:...] tag from the response.
    ...
    /// Parses a [POINT:x,y:label:screenN] or [POINT:none] tag from the end of the response.
