...
PATCH_INDEX_URLS = [
    ...
    "https://teamfighttactics.leagueoflegends.com/en-ph/news/game-updates/",
]
...
def fetch_latest_patch_url() -> str:
    ...
    for source_url in PATCH_INDEX_URLS:
        ...
        discovered_urls.extend(absolute_matches)

        relative_matches = re.findall(
            r"/[a-z-]+/news/game-updates/teamfight-tactics-patch-[^\"<\s]+/?",
            html,
            flags=re.IGNORECASE,
        )
        discovered_urls.extend(f"https://teamfighttactics.leagueoflegends.com{path}" for path in relative_matches)

    if not discovered_urls:
        raise RuntimeError("Could not find a TFT patch URL from index pages.")

    # Keep original order and deduplicate.
    deduped = list(dict.fromkeys(discovered_urls))
