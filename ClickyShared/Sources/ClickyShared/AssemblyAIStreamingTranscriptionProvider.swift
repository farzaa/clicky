...
//  ClickyShared
//
//  Streaming transcription provider backed by AssemblyAI's websocket API.
//
//  This provider fetches a short-lived temp token from the Cloudflare Worker
//  proxy (so the real AssemblyAI key never ships in the app), opens a v3
//  websocket using only that token, and forwards mic PCM16 audio frames as
//  they arrive.
//
//  Lifted from the open-source Clicky macOS app. The only iOS-relevant
//  adaptation is that the token proxy URL is now injectable via init
//  (instead of a hardcoded static), so the host app can point to its own
//  Worker without editing the package.
//

import AVFoundation
import Foundation

public struct AssemblyAIStreamingTranscriptionProviderError: LocalizedError {
...
public final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    /// URL for the Cloudflare Worker endpoint that returns a short-lived
    /// AssemblyAI streaming token. Defaults to `WorkerConfiguration.transcribeTokenProxyURL`.
    private let tokenProxyURL: String

    public let displayName = "AssemblyAI"
    public let requiresSpeechRecognitionPermission = false

    public var isConfigured: Bool { true }
    public var unavailableExplanation: String? { nil }
    ...
    public init(tokenProxyURL: String = WorkerConfiguration.transcribeTokenProxyURL) {
...
private final class AssemblyAIStreamingTranscriptionSession: NSObject, BuddyStreamingTranscriptionSession {
    ...
    private let stateQueue = DispatchQueue(label: "com.clicky.assemblyai.state")
    private let sendQueue = DispatchQueue(label: "com.clicky.assemblyai.send")
