...
//  ClickyShared
//
//  Claude API client with vision + streaming (SSE) support. Lifted from the
//  open-source Clicky macOS app and made cross-platform (Foundation only).
//
...
public final class ClaudeAPI {
    ...
    public init(proxyURL: String, model: String = "claude-sonnet-4-6") {
    ...
    /// Screen captures are typically JPEG, but pasted/imported images can be PNG.
    /// The API rejects requests where the declared media_type doesn't match
    ...
    /// Calls `onTextChunk` each time new text arrives so the UI updates progressively.
    /// Returns the full accumulated text and total duration when the stream completes.
    public func analyzeImageStreaming(
    ...
    /// Non-streaming fallback for cases where progressive display isn't needed.
