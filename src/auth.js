// Auth module — OIDC flow, token exchange, refresh timer
 ...
 * 5. Store token and schedule refresh
 *
...
function scheduleRefresh(expiresIn) {
  ...
  const refreshInMs = Math.max((expiresIn - 60) * 1000, 10000);
  refreshTimer = setTimeout(async () => {
    try {
      await authenticate();
      console.log("[auth] Token refreshed");
    } catch (err) {
      console.error("[auth] Token refresh failed, will need re-auth:", err);
      currentToken = null;
    }
  }, refreshInMs);
}
