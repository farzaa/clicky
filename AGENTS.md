...
| `DelegationLogSidebar.swift` | ~1190 | Right-edge floating log viewer for delegated agent runs. Coordinates multiple concurrent delegation sessions — every launched agent gets its own independent panel, log tail, and process-lifecycle watcher, stacked along the right edge. Each session has mac-style traffic lights: red closes the session, yellow minimizes it to a compact bar that can be clicked to restore, green is decorative. The panel uses a `NonKeyingBorderlessPanel` subclass (`canBecomeKey = false`) so clicking inside it never steals key-window state from the user's target app. For Claude Code runtimes, the sidebar feeds the raw log tail through a `DelegationLogClaudeStreamJSONParser` that understands Claude Code's `--output-format stream-json` frames (session init, streamed text deltas, tool uses, tool results, final result) and turns them into human-readable lines with live text-delta streaming. A pending-buffer flush runs at process exit so the final `result` frame appears even if it lands without a trailing newline. |
| `CodexExecLauncher.swift` | ~605 | Shared launcher service for Flowee's supported local coding-agent CLIs. Creates a fresh branch, launches the selected runtime in the chosen workspace, writes logs to disk, and returns branch + PR context for the delegation sidebar. When the caller supplies a semantic branch-name hint (e.g. `feature/dark-mode-settings`), the launcher sanitizes it, prefixes it with `flowee/`, checks for local collisions, and retries with a `-2`/`-3`/... numeric suffix before falling back to the legacy workspace-slug + timestamp + UUID format. |
