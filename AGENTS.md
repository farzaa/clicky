...
- **Analytics**: Local no-op tracker (`ClickyAnalytics.swift`) with no external telemetry
...
**Local-first configuration**: OpenRouter and ElevenLabs API keys are entered in-app and saved in macOS Keychain; model and voice preferences are saved in `UserDefaults`.
...
| `ClaudeAPI.swift` | ~300 | OpenRouter chat client with streaming (SSE) support and model list fetching. Includes TLS warmup and image payload handling. |
| `ElevenLabsTTSClient.swift` | ~95 | ElevenLabs TTS client. Sends text directly to ElevenLabs, plays audio via `AVAudioPlayer`, and exposes `isPlaying` for transient cursor scheduling. |
| `ElementLocationDetector.swift` | ~335 | Detects UI element locations in screenshots for cursor pointing. |
| `DesignSystem.swift` | ~880 | Design system tokens — colors, corner radii, shared styles. All UI references `DS.Colors`, `DS.CornerRadius`, etc. |
| `ClickyAnalytics.swift` | ~20 | Local analytics no-op shim so no external telemetry calls are made. |
...
Configure OpenRouter and ElevenLabs from the app panel `Settings` tab. Keys are saved in macOS Keychain and never committed to the repository.
