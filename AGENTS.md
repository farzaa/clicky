...
- **Background Agent Tasks**: Substantial requests ("reimplement this paper", "build me a React app") are classified by an LLM planner and handed off to the user's locally-installed Claude Code CLI. Dot is the orchestrator — voice trigger, planner, working-dir setup, panel UI, lifecycle, follow-ups, cancel. The actual coding work happens in a subprocess. Tasks live in `~/Desktop/Dot Tasks/<slug>-<date>/` with auto-`git init`. The right-edge panel surfaces live events streamed from Claude Code's `stream-json` output. Mid-run voice ("actually use JAX") routes as a follow-up; short cancellation phrases ("cancel that") kill the task. See `docs/agent-tasks-design.md` and the `AgentTaskOrchestration/` directory.
...
**Multi-turn agent loop**: After the user releases push-to-talk, `CompanionManager.sendTranscriptToClaudeWithScreenshot` runs a multi-step tool-use loop of up to `maxAgentStepsPerUserTurn` (10) steps. The loop sends the screenshot + transcript + tool schemas to Claude; the response is a list of text content blocks (TTS narration) and tool_use blocks (structured action calls). The controller executes each tool_use (clicks/keystrokes/navigation/etc.), captures the post-action screen, and sends `tool_result` blocks + the new image back as the next user message — Anthropic-native multi-turn. The loop ends when Claude returns a turn with no tool_use blocks (task complete), the `bail_out` tool is invoked, the user moves the hardware mouse > 40pt during the inter-step settling window (treated as the user reclaiming control — the baseline is re-captured after every step's actions), the step budget is exhausted, or push-to-talk re-fires. Per-step text is enqueued onto a serial TTS playback queue so the user hears narration in real time as actions execute. A 500ms settling delay between steps lets animations and page loads land; `navigate_browser` and `open_new_tab` add an extra 1.5s post-return wait specifically for page loads. System prompt + tool schemas are marked cacheable via `cache_control: ephemeral` so the static prefix amortizes across steps within a turn.

**Background agent dispatch**: Every finalized transcript that the direct-media short-circuit declines first runs through `AgentTaskPlanner.classifyAndPlan`, which calls Anthropic via vibe-id with a small classifier prompt and returns either `routeToInline` (existing tool-use loop) or `routeToBackground(brief:)`. Background briefs get a working dir under `~/Desktop/Dot Tasks/`, an `INSTRUCTIONS.md` written from the brief, and a fresh `git init`. `AgentTaskManager` then spawns `ClaudeCodeAdapter`, which invokes `claude -p --input-format=stream-json --output-format=stream-json --permission-mode=acceptEdits --max-turns N` and translates each stdout line into `AgentTaskEvent`s the right-edge `AgentTaskPanelView` renders. Only one task runs concurrently in v1; new requests during a running task become follow-ups (delivered via stdin JSON line) or cancels (short phrases like "cancel that"). Authentication is the user's responsibility — they must have run `claude` locally at least once to sign in or set `ANTHROPIC_API_KEY`.

## Key Files

| File | Lines | Purpose |
|------|-------|---------|
| `leanring_buddyApp.swift` | ~140 | Menu bar app entry point. Uses `@NSApplicationDelegateAdaptor` with `CompanionAppDelegate` which creates `MenuBarPanelManager`, the right-edge `AgentTaskPanelManager`, and starts `CompanionManager`. No main window — the app lives entirely in the status bar. |
| `CompanionManager.swift` | ~2470 | Central state machine. Owns dictation, shortcut monitoring, screen capture, Claude tool-use agent loop, ElevenLabs TTS (per-step narration queue), overlay management, direct media-command short-circuit, stuck-state recovery, permission state, and development diagnostics. Holds the `AgentTaskManager` instance and wires its announcement handler into TTS. Routes finalized transcripts through `AgentTaskPlanner` → either inline tool-use loop or background-task spawn. Tracks voice state (idle/listening/processing/responding), conversation history, model selection, and cursor visibility. |
| `AgentTaskOrchestration/AgentTaskBrief.swift` | ~120 | Core value types for the background-task pipeline: `AgentTaskBrief` (planner output), `AgentTaskEvent` (worker progress), `AgentTaskStatus` (lifecycle), `AgentTask` (ObservableObject holding brief + events + status), `TimestampedAgentTaskEvent`. |
...
| `AgentTaskOrchestration/ClaudeCodeAdapter.swift` | ~660 | `AgentWorker` conformer for Anthropic's `claude` CLI. Resolves the binary from common npm/Homebrew/Volta install paths (GUI apps don't inherit the interactive shell's PATH). Spawns `claude -p --input-format=stream-json --output-format=stream-json --permission-mode=acceptEdits --max-turns N` with a working directory. Parses each stdout JSON line into typed `AgentTaskEvent`s. Forwards mid-run follow-ups by writing JSON user-message lines to stdin. Cancels via SIGTERM with a SIGKILL escalation after 2s. |
| `AgentTaskOrchestration/AgentTaskManager.swift` | ~310 | Single-concurrent-task lifecycle manager (`@MainActor`, `ObservableObject`). Validates worker installation, creates the working dir under `~/Desktop/Dot Tasks/<slug>-<date>/`, spawns the adapter, consumes its event stream, updates `currentTask` + `recentlyFinishedTasks`, enforces the wall-clock budget watchdog, and fires `AgentTaskAnnouncement`s back to its host (CompanionManager) for TTS. |
| `AgentTaskOrchestration/AgentTaskPlanner.swift` | ~215 | Stateless classifier that calls Anthropic via vibe-id `/chat` with a small system prompt and returns `routeToInline` or `routeToBackground(brief:)`. Inputs the raw voice transcript; outputs JSON parsed into an `AgentTaskBrief`. Defensively strips code-fence wrapping if the model adds one. |
| `AgentTaskOrchestration/AgentTaskPanelManager.swift` | ~165 | Owns the right-edge floating `NSPanel` hosting `AgentTaskPanelView`. Same nonactivating-floating-canJoinAllSpaces pattern as `MenuBarPanelManager`. Subscribes via Combine to `AgentTaskManager.$currentTask` so it auto-shows when a task starts and auto-hides 7s after the task ends. Positions on the right edge of whichever screen contains the cursor. |
| `AgentTaskOrchestration/AgentTaskPanelView.swift` | ~420 | SwiftUI panel content: header with close button, current-task card (status badge, title, estimated time, most-recent assistant message, expandable event log of the last 40 events, cancel + reveal-in-Finder buttons), and a "recently finished" list of compact rows. Uses `DS` design system tokens. |
