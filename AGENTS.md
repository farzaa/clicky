...
- **Backend Web UI**: Minimal server-rendered FastAPI website (`/` + `/web/*`) with cookie-backed login/register/logout, workspace creation, VFS listing/upload, and lessons table visibility
...
| `backend/app/webui_router.py` | ~450 | Tiny server-rendered web UI routes (`/`, `/web/login`, `/web/register`, `/web/workspaces`, `/web/app`, `/web/upload`) with cookie sessions and direct Postgres-backed workspace/lesson dashboard rendering. |
