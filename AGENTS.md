...
| `CompanionManager.swift` | ~2200 | Central state machine. Owns dictation, shortcut monitoring, screen capture, Claude tool-use agent loop, ElevenLabs TTS (per-step narration queue), overlay management, direct media-command short-circuit, stuck-state recovery, permission state, and development diagnostics. Tracks voice state (idle/listening/processing/responding), conversation history, model selection, and cursor visibility. Coordinates the full push-to-talk → optional local media control → screenshot → Claude tool-use loop → per-step TTS → pointing/click/type/media pipeline. |
