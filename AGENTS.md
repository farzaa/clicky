...
- **Computer Control Tools**: `click_element`, `type_text`, `fill_text_field`, `press_keystroke`, `run_local_command`, `create_zip_archive`, `choose_file_or_folder`, `get_foreground_document_context`, `open_local_path`, `wait_for_seconds`, `navigate_browser`, `open_new_tab`, `close_tab`, `switch_tab`, `browser_back`, `browser_forward`, `media_control`, `point_at_element`, `bail_out`. Schemas in `AgentToolDefinitions.swift`. `CompanionComputerController` posts local CGEvents for click/type/keystroke, opens local paths in apps such as Cursor, and emits system-defined media-key events, while `CompanionManager` executes bounded local commands, structured zip creation, foreground document/page/PDF context capture, standard macOS file-picker selection, and short explicit waits directly. Obvious media-only phrases such as "pause the music" execute locally before the screenshot/model path. Synthetic clicks send `mouseMoved → 25 ms → mouseDown → 40 ms → mouseUp → 40 ms → warp` so Electron apps (Slack/Discord/VS Code) actually transfer focus to the element underneath — naive 0-duration down+up doesn't.
...
- **Background Agent Tasks**: Explicit `dot agent ...` requests are handed off to the user's locally-installed Claude Code CLI. Non-prefixed requests stay in Dot's normal live computer-use loop. Dot is the orchestrator — voice trigger, working-dir setup, panel UI, lifecycle, cancel, and delete. The actual work happens in a subprocess. Tasks live in `~/Desktop/Dot Tasks/<slug>-<date>/` with auto-`git init`. The right-edge dot overlay surfaces one colored dot per running/recent task; clicking a dot opens live events streamed from Claude Code's `stream-json` output. Short cancellation phrases ("cancel that") kill the most recent running task. See `docs/agent-tasks-design.md` and the `AgentTaskOrchestration/` directory.
...
**Background agent dispatch**: Background coding agents are explicit only. Every finalized transcript that the direct-media short-circuit declines goes to the normal live inline tool-use loop unless it starts with `dot agent ...`. No LLM route classifier decides whether to spawn a worker. Background requests become an `AgentTaskBrief`, get a working dir under `~/Desktop/Dot Tasks/`, an `INSTRUCTIONS.md` written from the request plus best-effort foreground page/PDF/document context, and a fresh `git init`. Existing absolute or `~/...` paths in the request are passed to Claude Code with `--add-dir` so project-folder work can inspect real files. If an explicit `dot agent` request asks for Cursor, Dot opens the generated task folder in a new Cursor window before the worker starts. Explicit coding agents run headless with code/search/shell tools and use `--permission-mode=bypassPermissions` with an explicit allowed-tool set so verification does not stall on invisible approval prompts. `AgentTaskManager` can run up to 5 concurrent agents; each task owns its own `ClaudeCodeAdapter`, event-consumer task, and wall-clock watchdog. Short cancellation phrases cancel the most recently started running task. Other non-prefixed transcripts stay inline. The transparent right-edge subagent dot overlay shows one colored dot per running/recent task; clicking a dot opens `AgentTaskPanelView` for that task's output. Hovering a dot reveals an `x` that deletes the subagent from Dot; deleting a running subagent cancels its worker but leaves the working directory on disk. Authentication is the user's responsibility — they must have run `claude` locally at least once to sign in or set `ANTHROPIC_API_KEY`.
...
| `AgentTaskOrchestration/ClaudeCodeAdapter.swift` | ~660 | `AgentWorker` conformer for Anthropic's `claude` CLI. Resolves the binary from common npm/Homebrew/Volta install paths (GUI apps don't inherit the interactive shell's PATH). Spawns `claude -p --input-format=stream-json --output-format=stream-json --permission-mode=bypassPermissions --max-turns N` with a working directory, a headless code/search/shell allowed-tool list, plus `--add-dir` for user-supplied paths detected in background requests. The bypass mode is intentionally restricted to explicit background coding agents so they can run verification scripts without invisible approval prompts. Parses each stdout JSON line into typed `AgentTaskEvent`s. Forwards mid-run follow-ups by writing JSON user-message lines to stdin. Cancels via SIGTERM with a SIGKILL escalation after 2s. |
...
| `AgentToolDefinitions.swift` | ~660 | Tool schemas (JSON Schema input contracts + Swift decode types) sent to Anthropic's Messages API. Defines the live agent tools, including UI control, browser navigation, bounded local command execution, structured zip creation, foreground document/page/PDF context capture, local path opening, macOS file-picker selection, media control, pointing, bailout, and memory support. Single source of truth for the tool catalog. |
...
| `OverlayWindow.swift` | ~970 | Full-screen transparent overlay hosting the blue cursor, response text, waveform, and animated hourglass waiting state. Handles cursor animation, element pointing with bezier arcs, multi-monitor coordinate mapping, and fade-out transitions. |
...
# falls back to ad-hoc signing. The script compiles from build/source-snapshot
# so open Xcode windows or other agents touching source mtimes do not break
# swiftc with "input file was modified during the build".
